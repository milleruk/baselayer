...
from .models import Challenge, ChallengeWorkoutAssignment, ChallengeWeekUnlock, ChallengeBonusWorkout
...
def admin_challenges_list(request):
    ...
    challenges = Challenge.objects.all().order_by("start_date", "created_at")  # Earliest first
    ...
    return render(request, "tracker/admin/challenges_list.html", {
...
def admin_challenge_create(request):
    ...
    if request.method == "POST":
        try:
            ...
            return redirect("tracker:admin_assign_workouts", challenge_id=challenge.id)
    ...
    return render(request, "tracker/admin/challenge_form.html", {
        "challenge": None,
        "templates": templates,
        "form_action": "create",
    })
...
def admin_challenge_edit(request, challenge_id):
    ...
    if request.method == "POST":
        try:
            ...
            return redirect("tracker:admin_challenges_list")
        except ValidationError as e:
            for field, errors in e.message_dict.items():
                for error in errors:
                    messages.error(request, f"{field}: {error}")
...
def admin_challenge_delete(request, challenge_id):
    ...
    return render(request, "tracker/admin/challenge_delete.html", {
        "challenge": challenge,
    })
...
def admin_assign_workouts(request, challenge_id):
    ...
    # day_labels is no longer used - we calculate day_label dynamically based on workout_day_number
    
    if not templates.exists():
        messages.warning(request, "No templates available for this challenge. Please add templates first.")
        return redirect("tracker:admin_challenge_edit", challenge_id=challenge_id)
    ...
    for template in templates:
        ...
        for week_num in range(1, num_weeks + 1):
            ...
            for day_num in range(7):
                ...
                # 3 rides/runs: Day 2 and Day 3 (2nd and 3rd workout days)
                # 4 rides/runs: Day 1, 4, and 6 (1st, 4th, and 6th workout days)
                # 2 runs 2 rides: Day 2 and Day 4 (2nd and 4th workout days)
                ...
                if workout_day_number:
                    ...
                    # Fallback to day name for rest days
    ...
    # Get bonus workouts for each week (only one per week)
    # Determine default activity type: run for "3 Runs a Week", ride for everything else
    ...
    if request.method == "POST":
        ...
        for template in templates:
            ...
            # Get workout day numbers for this template
            template_days = {d.day_of_week: d for d in template.days.all()}
            workout_days = []
            for d in range(7):
                td = template_days.get(d)
            ...
            for week_num in range(1, num_weeks + 1):
                for day_num in range(7):
                    ...
                    for activity_type in ["ride", "run", "yoga", "strength"]:
                        # Process primary workout (index 0)
                        ...
                        # Parse points, default to 50 if invalid
                        try:
                        ...
                        if peloton_url:
                            ...
                            # Remove primary and all alternatives if primary is empty
                        ...
                        # Process alternatives (only if alternatives are allowed)
                        if allows_alternatives:
                            alternative_index = 1
                            while alternative_index <= 10:  # Safety limit
                                ...
                                if peloton_url:
                                    # Use day_num as alternative_group ID
                                    assignment, created = ChallengeWorkoutAssignment.objects.update_or_create(
                                        challenge=challenge,
                                        template=template,
                                        week_number=week_num,
                                    ...
                                    # Remove this specific alternative if URL is empty
                                    ChallengeWorkoutAssignment.objects.filter(
                                        challenge=challenge,
                                        template=template,
                                        week_number=week_num,
                                    ...
                                    # Stop if we hit an empty alternative (assumes sequential)
                                    break
                                
                                alternative_index += 1
        
        # Handle bonus workouts (same for all templates)
        # Only one bonus workout per week - determine activity type based on challenge
        challenge_name_lower = challenge.name.lower()
        default_bonus_activity = "run" if "3 runs" in challenge_name_lower or ("run" in challenge_name_lower and "3" in challenge_name_lower) else "ride"
        ...
        for week_num in range(1, num_weeks + 1):
            ...
            # Create or update bonus workout (URL can be empty for "Any 30 min+ Peloton Workout")
            if workout_title or peloton_url:
                ...
                # Remove bonus workout if both URL and title are empty
    ...
    return render(request, "tracker/admin/assign_workouts.html", {
        "challenge": challenge,
        "workout_structure": workout_structure,
        # day_labels removed - using workout_day_number for day labels instead
